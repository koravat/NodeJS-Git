...
$('.container').ready(function(){
    key = $('#key').val();
	check_null_key(key)
    var firebaseRef = firebase.database().ref("Promotion/"+key);
    ...
    var storageRef = firebase.storage().ref("Promotion/"+key);
...
function editpromo(){
	...
	if(title1!=""&&description!=""&&start!=""&&end!="")
	{
		...
		else if(check1[2]<=check2[2]&&check1[1]<=check2[1]&&check1[0]<=check2[0])
		{
			...
			var firebaseRef=firebase.database().ref("Promotion/"+key).set({
			title : title1 ,
			desc : description ,
			start : start,
			end : end,
			...
			swal("Edit Promotion Success!", "", "success").then((result) => {
                if (result.value) {
                    storeProfile(uid);
                }
                else{
